...
app.get("/",(req,res)=>res.send("Tesla TurOptimal V2.0 Intelligence backend"));
app.get("/health",(req,res)=>res.json({
  ok:true, version:"2.0-intelligence",
  client:!!CLIENT_ID, secret:!!CLIENT_SECRET, google:!!GOOGLE_API_KEY,
  backendUrl:BACKEND_URL, appUrl:APP_URL,
  endpoints:["/auth/tesla","/api/tesla-live","/api/address-suggest","/api/route-intelligence","/api/wake"]
}));
...
app.get("/auth/callback",async(req,res)=>{
 try{
  ...
  const r=await fetch(`${TESLA_AUTH}/oauth2/v3/token`,{
   method:"POST",
   headers:{"Content-Type":"application/x-www-form-urlencoded",Accept:"application/json","User-Agent":"TeslaTurOptimal/2.0"},
   body
  });
...
async function teslaFetch(path,opt={}){
 ...
 const r=await fetch(`${TESLA_API}${path}`,{
  ...opt,
  headers:{Authorization:`Bearer ${tk}`,Accept:"application/json","Content-Type":"application/json",...(opt.headers||{})}
 });
 const{json,raw}=await safeJson(r);
...
app.get("/api/tesla-live",async(req,res)=>{
 try{
  ...
  const r=d.response||{}, c=r.charge_state||{}, dr=r.drive_state||{}, vs=r.vehicle_state||{}, cl=r.climate_state||{}, cfg=r.vehicle_config||{};
  ...
  res.json({ok:true,connected:true,
   vehicle:{id,name:v.display_name||v.vehicle_name||vs.vehicle_name||"Tesla",state:v.state||r.state||null,vin:r.vin||null,carVersion:vs.car_version||null,carType:cfg.car_type||null,wheelType:cfg.wheel_type||null,odometerKm:vs.odometer?vs.odometer*1.60934:null},
   telemetry:{batteryLevel:c.battery_level??null,usableBatteryLevel:c.usable_battery_level??null,chargeLimitSoc:c.charge_limit_soc??null,idealRangeKm:c.ideal_battery_range?c.ideal_battery_range*1.60934:null,ratedRangeKm:c.battery_range?c.battery_range*1.60934:null,chargingState:c.charging_state??null,chargerPowerKw:c.charger_power??null,chargePortOpen:c.charge_port_door_open??null,speedKmh,speedText:speedKmh===null?"Parkert / ikke rapportert":`${Math.round(speedKmh)} km/t`,powerKw:dr.power??null,latitude:dr.latitude??null,longitude:dr.longitude??null,shiftState:dr.shift_state??null,outsideTemp:cl.outside_temp??null,insideTemp:cl.inside_temp??null,climateOn:cl.is_climate_on??null,tpmsAvgBar:vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null,tpms,tpmsRecommended:{front:vs.tpms_rcp_front_value??null,rear:vs.tpms_rcp_rear_value??null},timestamp:new Date().toISOString()}
  });
...
async function googleRoute(points){
 ...
 const r=await fetch("https://routes.googleapis.com/directions/v2:computeRoutes",{
  method:"POST",
  headers:{"Content-Type":"application/json","X-Goog-Api-Key":GOOGLE_API_KEY,"X-Goog-FieldMask":"routes.legs.distanceMeters,routes.legs.duration,routes.polyline.encodedPolyline"},
...
function sample(a,max=220){if(!a||a.length<=max)return a||[];let o=[];for(let i=0;i<max;i++)o.push(a[Math.round(i*(a.length-1)/(max-1))]);return o}
...
function chargePowerKw(soc){
 if(soc<10)return 110;
 if(soc<25)return 170;
 if(soc<45)return 145;
 if(soc<60)return 105;
 if(soc<70)return 75;
 if(soc<80)return 48;
 return 28;
}
function minutesToCharge(fromSoc,toSoc,batteryKwh){
 let min=0;
 for(let s=fromSoc;s<toSoc;s+=1){
  const kwh=batteryKwh*0.01;
  min += (kwh/chargePowerKw(s))*60;
 }
 return min;
}
function consumptionWhKm(baseWhKm, leg){
 let wh=baseWhKm;
 if(leg.weather?.temperature<3)wh*=1.12; else if(leg.weather?.temperature<8)wh*=1.06;
 if(leg.weather?.rain>0.2)wh*=1.07;
 if(leg.weather?.windType==="motvind")wh*=1.12;
 if(leg.weather?.windType==="sidevind")wh*=1.04;
 if(leg.weather?.windType==="medvind")wh*=0.96;
 return wh;
}

function buildLegSegments(legs){
 let out=[],kmCursor=0;
 legs.forEach(l=>{
  const chunks=Math.max(1,Math.ceil(l.km/10));
  for(let i=0;i<chunks;i++){
   const km=l.km/chunks;
   out.push({km,from:l.from,to:l.to,legIndex:l.index,weather:l.weather,kmStart:kmCursor,kmEnd:kmCursor+km});
   kmCursor+=km;
  }
 });
 return out;
}
function interpolateMapPoint(mapPoints,totalKm,targetKm){
 const idx=Math.round((targetKm/Math.max(1,totalKm))*(mapPoints.length-1));
 return mapPoints[Math.max(0,Math.min(mapPoints.length-1,idx))]||mapPoints[0];
}
function optimizeCharging({segments,mapPoints,totalKm,batteryKwh,startSoc,minArrivalSoc,baseWhKm}){
 let soc=startSoc;
 let km=0;
 let stops=[];
 let i=0;
 const buffer=minArrivalSoc;
 while(i<segments.length){
  const seg=segments[i];
  const whKm=consumptionWhKm(baseWhKm,seg);
  const needSoc=(seg.km*whKm/1000)/batteryKwh*100;
  if(soc-needSoc<buffer){
   const p=interpolateMapPoint(mapPoints,totalKm,km);
   let bestTo=55, bestTotal=999999, bestReach=0;
   for(let to=45;to<=75;to+=5){
    if(to<=soc+3)continue;
    let tempSoc=to, reach=0, j=i;
    while(j<segments.length){
     const n=segments[j], nNeed=(n.km*consumptionWhKm(baseWhKm,n)/1000)/batteryKwh*100;
     if(tempSoc-nNeed<buffer)break;
     tempSoc-=nNeed; reach+=n.km; j++;
    }
    const chargeMin=minutesToCharge(Math.max(5,soc),to,batteryKwh);
    const score=chargeMin - Math.min(reach,220)*0.055 + (to>65?(to-65)*0.55:0);
    if(score<bestTotal){bestTotal=score;bestTo=to;bestReach=reach}
   }
   const chargeMin=Math.ceil(minutesToCharge(Math.max(5,soc),bestTo,batteryKwh)+3);
   stops.push({index:stops.length+1,lat:p.lat,lng:p.lng,approxAfterKm:Math.round(km),arriveSoc:Math.round(soc),chargeToSoc:bestTo,chargeMinutes:chargeMin,estimatedReachKm:Math.round(bestReach),type:"Tesla Supercharger først",comment:"Optimalisert for kort ladetid: unngår høy SOC og prioriterer rask ladezone."});
   soc=bestTo;
   continue;
  }
  soc-=needSoc; km+=seg.km; i++;
 }
 return {stops,arrivalSoc:Math.round(soc),totalChargeMinutes:stops.reduce((s,x)=>s+x.chargeMinutes,0),needed:stops.length>0,comment:stops.length?"Ladestopp er beregnet for lavest mulig total reisetid. Tesla Supercharger prioriteres først.":"Ingen lading nødvendig med valgt buffer."};
}

app.post("/api/route-intelligence",async(req,res)=>{
 try{
  const stops=(req.body.stops||[]).filter(Boolean).slice(0,20);
  const params=req.body.params||{}, tesla=req.body.tesla||{};
  ...
  const rawLegs=rt.legs||[];
  const poly=rt.polyline?.encodedPolyline?decodePolyline(rt.polyline.encodedPolyline):points.map(p=>({lat:p.lat,lng:p.lng}));
  const mapPoints=sample(poly,260);
  const els=await elevations(sample(poly,120));
  let totalUp=0,totalDown=0; for(let i=1;i<els.length;i++){const d=els[i]-els[i-1];if(d>0)totalUp+=d;else totalDown+=Math.abs(d)}
  const tire=tirePenalty(tesla);
  const batteryKwh=Number(params.batteryKwh||74);
  const baseWhKm=Number(params.baseWhKm||155);
  const regenEff=Number(params.regenEfficiency||65)/100;
  const startSoc=Number(tesla.telemetry?.usableBatteryLevel||tesla.telemetry?.batteryLevel||80);
  const minArrivalSoc=Number(params.minArrivalSoc||15);
  const mass=1750+Math.max(0,Number(params.passengers||2)-1)*80+(params.luggage==="tung"?90:params.luggage==="normal"?40:0);

  let totalKm=0,totalMinutes=0,baseKwh=0,weatherKwh=0,tireKwh=0,climbKwh=0,regenKwh=0,netKwh=0;
  const routeKm=rawLegs.reduce((s,l)=>s+((l.distanceMeters||0)/1000),0)||1;
  const legs=[],weatherPoints=[];
  for(let i=0;i<rawLegs.length;i++){
   const a=points[i],b=points[i+1];
   const km=(rawLegs[i].distanceMeters||hav(a,b)*1000)/1000;
   const dur=parseInt(String(rawLegs[i].duration||"0s").replace("s",""),10)||(km/75*3600);
   const minutes=dur/60;
   const mid={lat:(a.lat+b.lat)/2,lng:(a.lng+b.lng)/2};
   const w0=await weather(mid.lat,mid.lng);
   const w={...w0,windType:windType(bearing(a,b),w0.windDir)};
   const fac=weatherFactor(w);
   const share=km/routeKm, legUp=totalUp*share, legDown=totalDown*share;
   const legBase=km*baseWhKm/1000, legWeather=legBase*fac.extra, legTire=legBase*tire.penalty, legClimb=pe(legUp,mass), legRegen=pe(legDown,mass)*regenEff, legNet=Math.max(0,legBase+legWeather+legTire+legClimb-legRegen);
   ...
   const count=Math.max(1,Math.round(minutes/30));
   for(let p=0;p<count;p++)weatherPoints.push({timeLabel:new Date(Date.now()+(totalMinutes-minutes+p*30)*60000).toLocaleTimeString("no-NO",{hour:"2-digit",minute:"2-digit"}),place:`${stops[i]} → ${stops[i+1]}`,temperature:w.temperature,rain:w.rain,windSpeed:w.windSpeed,windType:w.windType});
  }

  const segments=buildLegSegments(legs);
  const charging=optimizeCharging({segments,mapPoints,totalKm,batteryKwh,startSoc,minArrivalSoc,baseWhKm});
  const arrivalSoc=charging.arrivalSoc;
  const totalTripMinutes=totalMinutes+charging.totalChargeMinutes;

  res.json({ok:true,plan:{version:"2.0-intelligence",params,stops,points,mapPoints,totalKm,totalMinutes,totalTripMinutes,baseKwh,weatherKwh,tireKwh,climbKwh,regenKwh,netKwh,elevationUp:totalUp,elevationDown:totalDown,arrivalSoc,tire,charging,legs,weatherPoints,intelligence:{strategy:"Minimer total reisetid, ikke færrest stopp. Lader helst i rask SOC-sone og prioriterer Tesla Supercharger.",startSoc,minArrivalSoc,batteryKwh,baseWhKm}}});
 }catch(e){res.status(500).json({ok:false,error:e.message})}
});
app.post("/api/route-clean",(req,res)=>{req.url="/api/route-intelligence";app._router.handle(req,res)});
app.listen(PORT,()=>console.log("Tesla TurOptimal V2.0 Intelligence backend on port "+PORT));
